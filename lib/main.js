...
function findName(requestName) {
	...
	for (i in jsonSong) {
		...
		if (jsonSong[i].url.indexOf(requestName, 0) >= 0) {
			...
			var name = utf8Converter.convertURISpecToUTF8(jsonSong[i].artist + "_" + jsonSong[i].title + ".mp4", "UTF-8");
			return name;
		}
	}
	return "unknown";
...
function save(name, data) {
	...
	aFile.initWithPath(require("sdk/simple-prefs").prefs.musicDirectory);
...
TracingListener.prototype = {
	...
	onStopRequest : function(request, context, statusCode) {
		...
		} else if (request.name.indexOf("mp4") == request.name.length - 3) {
			// it is a music
			var responseSource = this.receivedData.join("");
			//console.log("recieve mp4");
		...
		if (aActivityType == nsIHttpActivityObserver.ACTIVITY_TYPE_HTTP_TRANSACTION) {
			switch (aActivitySubtype) {
				case nsIHttpActivityObserver.ACTIVITY_SUBTYPE_REQUEST_HEADER:
					...
					if (aExtraStringData.indexOf('mp4', 0) > 0 && aExtraStringData.indexOf('except_report', 0) < 0) {
						// console.log(aExtraStringData);
						tmp = aExtraStringData.substring(aExtraStringData.indexOf(".mp4") - 7, aExtraStringData.indexOf(".mp4"));
						// console.log(tmp);
...
pageMod.PageMod({
	...
	onAttach : function onAttach(worker) {
		if (registered == false) {
			statusPanel = require("sdk/panel").Panel({
				// width: 300,
				// height: 100,
			...
			if (testIfSaveDirExisted()) {
				statusWidget = require("sdk/widget").Widget({
					label : "DoubanFm",
					id : "DoubanFm",
				...
				statusPanel.on("show", function() {
					if (widgetClicked) {
						...
						nameList = listFiles(require("sdk/simple-prefs").prefs.musicDirectory);
